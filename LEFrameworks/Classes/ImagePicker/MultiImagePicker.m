...
#import "MultiImagePicker.h"
#import "LEBaseTableView.h"
#import "LEBaseTableViewCell.h"
#import <AssetsLibrary/AssetsLibrary.h>

@interface MultiImagePickerFlowCell : UIImageView
@property (nonatomic) BOOL isChecked;
@end
@implementation MultiImagePickerFlowCell{
    UIImageView *curIcon;
    ALAsset *curAsset;
}
...
-(void) initUI{
    ...
    curIcon=[LEUIFramework getUIImageViewWithSettings:[[LEAutoLayoutSettings alloc] initWithSuperView:self Anchor:LEAnchorInsideBottomRight Offset:CGPointMake(-2, -2) CGSize:CGSizeZero] Image:[[LEUIFramework sharedInstance] getImageFromLEFrameworksWithName:@"MultiImagePickerCheck"]];
...
@interface MultiImagePickerFlowPage:LEEmptyViewUnderSystemNavigation
@end
@implementation MultiImagePickerFlowPage{
...
-(void) reloadCell{
    for (int i=0; i<curArray.count; i++) {
        MultiImagePickerFlowCell *cell=[[MultiImagePickerFlowCell alloc] initWithFrame:CGRectMake((cellSize+space)*(i%4), LayoutSideSpace+(cellSize+space)*(i/4), cellSize, cellSize)];
...
-(NSArray *) getData{
    ...
    for (int i=0; i<curCellCache.count; i++) {
        MultiImagePickerFlowCell *cell=[curCellCache objectAtIndex:i];
        if(cell.isChecked){
...
-(id) initWithViewController:(LEViewController *)vc AssetsGroup:(ALAssetsGroup *) group{
    curGroup=group;
    return [super initWithViewController:vc];
}
@end

@interface MultiImagePickerFlow:LEViewController
@end
@implementation MultiImagePickerFlow{
    BOOL isBarHide;
    MultiImagePickerFlowPage *page;
}
...
-(id) initWithDelegate:(id<LEViewControllerPageJumpDelagte>)delegate AssetsGroup:(ALAssetsGroup *) group{
    self=[super initWithDelegate:delegate];
    page=[[MultiImagePickerFlowPage alloc] initWithViewController:self AssetsGroup:group];
    return self;
}
@end

//==============================
@interface MultiImagePickerCell : LEBaseTableViewCell
@property (nonatomic) NSMutableArray *imagesAssetArray;
@end
@implementation MultiImagePickerCell{
    UIImageView *curIcon;
    UILabel *curTitle;
    UILabel *curSubtitle;
    int cellH;
...
@interface MultiImagePickerPage:LEEmptyViewUnderSystemNavigation<LETableViewCellSelectionDelegate,LEViewControllerPageJumpDelagte>
@property (nonatomic) ALAssetsLibrary *assetsLibrary;
@property (nonatomic) NSMutableArray *albumsArray;
@end
@implementation MultiImagePickerPage{
    LEBaseTableView *curTableView;
    NSMutableArray *curArray;
    id<MultiImagePickerDelegate> curDelegate;
}
-(id) initWithViewController:(LEViewController *)vc Delegate:(id<MultiImagePickerDelegate>) delegate{
...
-(void) setExtraViewInits{
    ...
    curArray=[[NSMutableArray alloc] init];
    curTableView=[[LEBaseTableView alloc] initWithSettings:[[LETableViewSettings alloc] initWithSuperViewContainer:self ParentView:self.viewContainer TableViewCell:@"MultiImagePickerCell" EmptyTableViewCell:nil GetDataDelegate:nil TableViewCellSelectionDelegate:self]];
...
-(void) onTableViewCellSelectedWithInfo:(NSDictionary *)info{
    ...
    MultiImagePickerFlow *vc=[[MultiImagePickerFlow alloc] initWithDelegate:self AssetsGroup:[[self.albumsArray objectAtIndex:index.row] objectForKey:@"group"]];
...
@interface MultiImagePicker ()
@end
@implementation MultiImagePicker{
    BOOL isBarHide;
    MultiImagePickerPage *page;
}
...
-(id) initWithImagePickerDelegate:(id<MultiImagePickerDelegate>) delegate{
    self=[super init];
    page=[[MultiImagePickerPage alloc] initWithViewController:self Delegate:delegate];
    return self;
}
@end
